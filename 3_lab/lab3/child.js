...
process.on('message', (obj) => {
    ...
    /* Свойство expression содержит строку, которую будем передавать на вход в нейронную сеть */
    const output = net.run(input);
    /* метод run позволяет передать на вход в нейронную сеть строку и получить результат работы нейронной сети */
    console.log('Child: ' + input + output);
    obj.result = input + output;
    process.send(obj);
    /*методу send передается объект, который будет передан родительскому процессу */
});
